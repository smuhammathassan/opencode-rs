...
pub fn def() -> crate::tool::tool::Def {
    crate::tool::tool::def("lsp", prompts::LSP, parameters(), |args, ctx| {
        let operation = args
            .get("operation")
            .and_then(|v| v.as_str())
            .unwrap_or("")
        ...
        // TODO(integration): dispatch the LSP request through `oc-lsp` and
        // render the result; the reference returns a pretty JSON array.
