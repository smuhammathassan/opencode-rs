...
impl Worktree {
    ...
    async fn candidate(
        ...
        Err(WorktreeError::name_generation_failed(
    ...
    pub async fn make_worktree_info(
        ...
        if ctx.project.vcs.as_deref() != Some("git") {
            return Err(WorktreeError::not_git(
                "Worktrees are only supported for git projects",
            ));
        }
        let root = pathutil::join(&[
    ...
    async fn setup(&self, ctx: &InstanceContext, info: &WorktreeInfo) -> Result<(), WorktreeError> {
        ...
        let _ = self
            .project
            .add_sandbox(&ctx.project.id, &info.directory)
            .await;
        Ok(())
    }
    ...
    async fn boot(&self, ctx: &InstanceContext, info: &WorktreeInfo, start_command: Option<&str>) {
        ...
        let booted = match self
            .store
            .load(crate::project::instance_store::LoadInput::directory(
                &info.directory,
            ))
            .await
        {
        ...
        self.run_start_scripts(&info.directory, &project_id.0, &extra)
            .await;
    }

    fn emit_worktree_event(
        &self,
        directory: &str,
        project_id: &str,
    ...
    pub async fn create(
        &self,
        ctx: &InstanceContext,
        input: Option<&WorktreeCreateInput>,
    ) -> Result<WorktreeInfo, WorktreeError> {
        let info = self
            .make_worktree_info(
                ctx,
                &WorktreeInfoOptions {
                    name: input.and_then(|i| i.name.clone()),
                    detached: false,
                },
            )
            .await?;
        self.create_from_info(
            ctx,
            &info,
            input.and_then(|i| i.startCommand.clone()).as_deref(),
        )
        .await?;
        Ok(info)
    }
    ...
    pub async fn list(&self, ctx: &InstanceContext) -> Result<Vec<WorktreeInfo>, WorktreeError> {
        ...
        let result = self
            .git(&["worktree", "list", "--porcelain"], Some(&ctx.worktree))
            .await;
        if result.code != 0 {
        ...
        for entry in parse_worktree_list(&result.text) {
            ...
            let name = if name == primary_name {
                pathutil::basename(&pathutil::dirname(&directory))
            } else {
                name
            };
            out.push(WorktreeInfo {
                name,
                directory,
                branch: entry.branch.map(|branch| {
                    branch
    ...
    async fn clean_directory(&self, target: &str) -> Result<(), WorktreeError> {
        ...
        if fs::exists(target).await {
            Err(WorktreeError::remove_failed(
    ...
    pub async fn remove(
        ...
        if removed.code != 0 {
            ...
            let stale = self
                .locate_worktree(&parse_worktree_list(&next.text), &directory)
                .await;
        ...
        {
            let deleted = self
                .git(&["branch", "-D", branch], Some(&ctx.worktree))
                .await;
            if deleted.code != 0 {
                let message = fallback(
                    &deleted.stderr,
    ...
    async fn run_start_script(&self, directory: &str, cmd: &str, kind: &str) -> bool {
        ...
        tracing::error!(
            "worktree start command failed: kind={kind} directory={directory} message={stderr}"
        );
        false
    }
    ...
    pub async fn reset(
        ...
        if directory == primary {
            return Err(WorktreeError::reset_failed(
        ...
        let entry = self
            .locate_worktree(&parse_worktree_list(&list.text), &directory)
            .await;
        let Some(entry) = entry else {
        ...
        let worktree_path = entry
            .path
            .ok_or_else(|| WorktreeError::reset_failed("Worktree not found"))?;
        ...
        if let Some(separator) = base.r#ref.find('/') {
            if base.r#ref != base.name {
                ...
                self.git_expect(&["fetch", remote, branch], &ctx.worktree, |result| {
                    WorktreeError::reset_failed(fallback(
                        &result.stderr,
                        &result.text,
                        &format!("Failed to fetch {}", base.r#ref),
                    ))
                })
                .await?;
            }
        ...
        tokio::spawn(async move {
            let _ = worktree
                .run_start_scripts(&worktree_path, &project_id, "")
                .await;
        });
...
mod tests {
    ...
    fn parse_worktree_list_parses_porcelain() {
        ...
        assert_eq!(
            ...
            Some("refs/heads/opencode/feature")
        );
    }
